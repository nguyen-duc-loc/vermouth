# Verify: Cloud deployment for friend testing · spec 0006 · updated 2026-09-04
...
- [ ] Change one production secret. Confirm a new content named Secret uses the first 12 characters of the canonical source SHA256, the old current or previous Secret remains, and an unreferenced older Secret is collected only after external readiness. → AC-12
...
- [ ] Inspect the immutable release identity. Confirm it combines the full Git SHA, GitHub run ID, and run attempt, and that Helm stores the same identity with every evidence checksum. → AC-6, AC-13, AC-20
...
- [ ] Create distinct markers in four databases, Redpanda, Garage metadata, Garage data, and Traefik ACME state. Run `task prod:export -- <new-path>`. Inspect the authenticated manifest and confirm it contains live storage, four custom dumps, Helm state, production configuration, and complete current plus optional previous release directories. → AC-17
...
- [ ] Complete restore after typed `vermouth`. Confirm all eight stateful markers, four logical dumps, current and previous immutable releases, Secrets, image digests, HTTPS paths, status resolution, rollback candidate, Google sign in, invoice flow, and event thread match the export. → AC-18
...
- [ ] Deploy two compatible application revisions, including two attempts for one Git SHA. Run `task prod:rollback`, confirm the newest earlier deployable Helm revision is selected from live history, and verify every Secret, evidence checksum, and live Goose version compatibility proof before mutation. Confirm the target image and Secret references are displayed before the remote command accepts typed `vermouth` while holding the production lock. → AC-14
