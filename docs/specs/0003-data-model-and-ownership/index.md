...
Every entity in Vermouth gets exactly one owning service and one owning table, and every copy of
someone else's fact gets named with the event that carries it. Spec 0001 already decided who owns
what, so this spec places the actual tables and columns against that map and writes one migration per
service. The two ideas that decide correctness: a copy (a projection) is rebuildable by replaying
events and is keyed by the id its events carry, while an authoritative record (an invoice, its
number, its paid state) is never rebuilt and no replay may touch it. Nothing is ever deleted, and no
copy stores a number it could count at read time.
...
- **AC-3**: Every projection column is named with the event that writes it, in the migration comment
  above its table, and no projection column exists that no listed event writes.
...
- **AC-12**: The invoice profile completeness gate exists once in the schema, and both the month end
  refusal and the profile screen read that one gate rather than each testing its own field list.
...
nullable end timestamp, and land the whole confirmed model as one migration per service
(basis: spec 0001's ownership map and local copies table, which this spec applies rather than
reopens).
...
does. Every table carries `tutor_id`, as a column or inside its key.
...
#### `teaching`, migration `00002_teaching_model.sql`

| Table | Key columns | Other columns | Constraints and indexes |
|---|---|---|---|
| `students` | `student_id uuid PK` | `tutor_id NOT NULL`, `name text NOT NULL`, `phone text` (nullable, stored exactly as typed), `created_at`, `updated_at`, `removed_at` | index `(tutor_id) WHERE removed_at IS NULL` |
| `classes` | `class_id uuid PK` | `tutor_id NOT NULL`, `name text NOT NULL`, `rate_amount bigint NOT NULL`, `currency text NOT NULL`, `rate_effective_from date NOT NULL`, `created_at`, `updated_at`, `archived_at` | `CHECK (currency = 'VND')`, `CHECK (rate_amount >= 0)`, index `(tutor_id) WHERE archived_at IS NULL` |
| `sessions` | `session_id uuid PK` | `class_id NOT NULL REFERENCES classes`, `tutor_id NOT NULL`, `starts_at NOT NULL`, `ends_at NOT NULL`, `local_date date NOT NULL`, `schedule_rule_id uuid` (nullable, no table yet), `created_at`, `updated_at`, `cancelled_at` | `CHECK (ends_at > starts_at)`, index `(tutor_id, local_date) WHERE cancelled_at IS NULL`, index `(class_id)` |
| `roster_periods` | `PK (class_id, student_id, effective_from)` | `tutor_id NOT NULL`, `effective_to date`, `created_at`, `updated_at`, plus `REFERENCES classes` and `REFERENCES students` | `UNIQUE (class_id, student_id) WHERE effective_to IS NULL`, `CHECK (effective_to IS NULL OR effective_to >= effective_from)` |
| `attendance` | `PK (session_id, student_id)` | `tutor_id NOT NULL`, `state text NOT NULL`, `marked_at timestamptz NOT NULL`, `created_at`, `updated_at`, plus `REFERENCES sessions` and `REFERENCES students` | `CHECK (state IN ('Present', 'Absent'))` |
...
#### `billing`, migration `00002_billing_model.sql`

The six projections first, then the six authoritative tables, in one file with a comment marking the
line between them.
...
| `class_rates` | `PK (class_id, effective_from)` | `tutor_id NOT NULL`, `rate_amount bigint NOT NULL`, `currency text NOT NULL CHECK (currency = 'VND')`, `recorded_at` | `teaching.class.created` seeds the first row, `teaching.class.rate.changed` appends |
...
| `billing_runs` | `billing_run_id uuid PK` | `tutor_id NOT NULL`, `period_year integer NOT NULL`, `period_month integer NOT NULL`, `generation integer NOT NULL`, `created_at`, `superseded_at` | `UNIQUE (tutor_id, period_year, period_month, generation)`, `CHECK (period_month BETWEEN 1 AND 12)`, `CHECK (generation >= 1)` |
| `invoices` | `invoice_id uuid PK` | `tutor_id NOT NULL`, `billing_run_id NOT NULL REFERENCES billing_runs`, `student_id NOT NULL`, `invoice_number text NOT NULL`, `period_year`, `period_month`, `total_amount bigint NOT NULL`, `currency text NOT NULL`, `issued_at NOT NULL`, `pdf_location text` (nullable), `paid_at`, `voided_at`, `void_reason text`, `replaces_invoice_id uuid REFERENCES invoices`, the frozen render block below, `updated_at` | `UNIQUE (tutor_id, invoice_number)`, `CHECK (currency = 'VND')`, `CHECK ((voided_at IS NULL) = (void_reason IS NULL))`, `CHECK (void_reason IS NULL OR void_reason <> '')`, index `(tutor_id, period_year, period_month)` |
| `invoice_lines` | `invoice_line_id uuid PK` | `invoice_id NOT NULL REFERENCES invoices`, `tutor_id NOT NULL`, `session_id uuid NOT NULL`, `session_date date NOT NULL`, `class_name text NOT NULL`, `rate_amount bigint NOT NULL`, `amount bigint NOT NULL` | `UNIQUE (invoice_id, session_id)`, index `(invoice_id)` |
...
| `FindOpenRosterPeriod` | `teaching` | `class_id`, `student_id` | the one open row | same | none. Absent means never joined or already left |
| `ListBillableSessions` | `billing` | `tutor_id`, `local_date` range | session, student, rate rows for the period | same | none |
| `RateInForceOn` | `billing` | `class_id`, `local_date` | newest `class_rates` row on or before the date | same | absent, which the run turns into `rate_missing` |
...
| `GetInvoiceProfile` | `billing` | `tutor_id` | fields plus `is_complete` | same | absent row, which cannot happen after registration |
...
|---|---|---|
| Any consumer upsert | The projection row's identity | The identifying id the event already carries: `student_id`, `class_id`, `session_id`, `(session_id, student_id)`, or `(class_id, student_id, effective_from)` |
...
| Any table | `tutor_id` | The `sub` claim of the verified token, never an input (INV-8) |
...
- No foreign key crosses a service boundary. Inside a service, a foreign key is used only where both
  tables are authoritative and in the same context; projections reference by id without one, because
  events for two different keys may arrive in either order.
- Money is `bigint` dong and currency is `VND` by `CHECK`.

**Security model**:

- `tutor_id` comes from the verified token and nothing else, and every query filters on it (INV-8).
  There is no cross tutor read path, and no table is reachable without the filter.
...
- Idempotent projection: the same `teaching.session.scheduled` delivered twice, then a full replay of
  ...
  **AC-9**.
- Replay safety: a replay of both consumed topics after an invoice exists leaves `invoices`,
  `invoice_lines`, `invoice_number_counters`, `billing_runs`, and the `invoice_profiles` field values
  byte for byte unchanged, verifies **AC-8**.
...
whole model come last as the thread's closing check. One migration per service, each landing that
service's whole confirmed model, because four files as the schema of record are easier to read than
twelve, and this feature's own "Done when" asks that each service's migration applies cleanly.
...
6. The first `db/queries/` file per service, covering the load bearing queries in the API surface
   table above, every one filtering by `tutor_id`, replacing the two `db/queries/README.md`
   placeholders, satisfies **AC-4**, **AC-5**
7. `task migrate:up` from empty databases, then `sqlc generate` and `task build` across every module,
   satisfies **AC-2**, **AC-5**
8. The query guard test: every statement on a tutor scoped table names `tutor_id`, and no statement
   names a table its service does not own, satisfies **AC-4**, **AC-6**
...
10. The replay and idempotency test against the real Postgres and Redpanda: a duplicate delivery and a
    full replay leave projections identical and authoritative records untouched, satisfies **AC-7**,
    **AC-8**, **AC-9**
11. The remaining scenario tests: concurrent run insert and concurrent number take, the rejoin case,
    and the completeness gate table, satisfies **AC-1**, **AC-11**, **AC-12**
