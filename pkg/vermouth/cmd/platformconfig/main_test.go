...
func TestRunRejectsInvalidCommandArguments(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "production image plan", args: []string{"production-image-plan", "root", "gateway"}, wantError: "usage: platformconfig production-image-plan"},
		{name: "production images document", args: []string{"production-images-document", "metadata"}, wantError: "usage: platformconfig production-images-document"},
		{name: "tree SHA256", args: []string{"tree-sha256"}, wantError: "usage: platformconfig tree-sha256"},
